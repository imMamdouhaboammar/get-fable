...
fn test_generate_suggested_test_for_categories() {
    let sql_test = generate_suggested_test("sql-injection", "http://localhost:3000/api/users", "SQL Injection");
    assert!(sql_test.contains("SQL injection probe safely rejected"));

    let cors_test = generate_suggested_test("cors-misconfiguration", "http://localhost:3000/api/data", "CORS Misconfig");
    assert!(cors_test.contains("CORS rejects unauthorized arbitrary origins"));

    let idor_test = generate_suggested_test("idor-bola", "http://localhost:3000/api/docs/123", "BOLA");
    assert!(idor_test.contains("rejects unauthorized cross-tenant access"));

    let headers_test = generate_suggested_test("security-headers", "http://localhost:3000", "Missing Headers");
    assert!(headers_test.contains("enforce security headers"));

    let sensitive_test = generate_suggested_test("sensitive-exposure", "http://localhost:3000/.env", "Leaked env");
    assert!(sensitive_test.contains("sensitive path"));
}

#[test]
fn test_synthesize_code_patch_sql_parameterization() {
    let vulnerable_code = "const result = await db.query(`SELECT * FROM users WHERE id = ${userId}`);";
