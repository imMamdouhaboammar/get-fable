...
impl CodexHostAdapter {
    ...
    pub fn detect(&self, facts: &MachineFacts) -> HostDetection {
        let host_fact = facts.hosts.iter().find(|h| h.id == "codex");
        ...
        HostDetection {
            ...
            config_root,
            project_config_root: Some(PathBuf::from(".codex")),
    ...
    pub fn capability_matrix(&self, _facts: &MachineFacts) -> HostCapabilityMatrix {
        ...
        features.insert("pre_tool_hook".into(), FeatureSupport::Unknown);
        features.insert("post_tool_hook".into(), FeatureSupport::Unknown);
    ...
    pub fn apply_patch(&self, plan: &HostPatchPlan) -> Result<HostBindingReceipt, EcoError> {
        ...
        let digest = match &plan.operation {
            ...
            _ => return Err(EcoError::Config("Unsupported operation for Codex".into())),
        };
