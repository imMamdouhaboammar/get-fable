...
`get-fable` 1.6.0 ships a skill-only OpenAI plugin (`.codex-plugin/plugin.json`) plus a native Claude Code plugin and marketplace manifest (`.claude-plugin/marketplace.json` and `.claude-plugin/plugin.json`). The universal semantic surface is the canonical lifecycle under `skills/`.
