...
import { validateFableState, workspaceIdForTarget } from '../../src/core/state.ts';
...
describe('repository state template portability', () => {
  test('tracked .fable/state.json is workspace-neutral and binds at runtime', () => {
    ...
    try {
      ...
      expect(stateA.workspaceId).toBe(workspaceIdForTarget(a));
      expect(stateB.workspaceId).toBe(workspaceIdForTarget(b));
    } finally {
