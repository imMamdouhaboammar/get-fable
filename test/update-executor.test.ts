...
function plan(overrides: Partial<UpdatePlan> = {}): UpdatePlan {
  return {
    ...
    strategy: 'npm-global',
    executable: 'npm',
    argv: ['install', '-g', 'get-fable@1.6.1'],
...
describe('explicit update executor', () => {
  test('executes the exact Bun argv from the validated plan', () => {
    ...
    expect(calls).toEqual([{ executable: 'bun', argv: ['add', '-g', 'get-fable@1.6.1'] }]);
  ...
  test('executes the exact npm argv from the validated plan', () => {
    ...
    expect(calls).toEqual([{ executable: 'npm', argv: ['install', '-g', 'get-fable@1.6.1'] }]);
    expect(receipt.success).toBe(true);
  });
  ...
  test('reports success only after the installed version matches exactly', () => {
    const receipt = executeUpdate(plan(), deps({ verifyInstalledVersion: () => '1.6.1' }));

    expect(receipt.success).toBe(true);
    expect(receipt.outcome).toBe('success');
    expect(receipt.verifiedVersion).toBe('1.6.1');
    expect(receipt.targetVersion).toBe('1.6.1');
  });
