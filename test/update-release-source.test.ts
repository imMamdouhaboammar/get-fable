...
describe('stable release source', () => {
  test('uses npm latest as authority and enriches the matching GitHub release', async () => {
        ...
        fetch: async (input) => {
          ...
          if (input === 'https://registry.npmjs.org/get-fable') {
            return jsonResponse({
              ...
              versions: {
                '1.8.0': { dist: { integrity: 'sha512-test-integrity' } },
              },
            });
          }
          if (input.endsWith('/releases/tags/v1.8.0')) {
            return jsonResponse({
              html_url: 'https://github.com/imMamdouhaboammar/get-fable/releases/tag/v1.8.0',
    ...
    expect(result.releaseUrl).toBe('https://github.com/imMamdouhaboammar/get-fable/releases/tag/v1.8.0');
  ...
  test('keeps a valid npm result when GitHub enrichment fails', async () => {
    const result = await fetchStableRelease('1.8.0', {
      now: () => new Date('2026-08-28T20:00:00.000Z'),
      fetch: async (input) => {
        if (input === 'https://registry.npmjs.org/get-fable') {
          return jsonResponse({ 'dist-tags': { latest: '1.8.0' }, versions: {} });
        }
